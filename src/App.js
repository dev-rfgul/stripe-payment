...
// export default App;
import React from "react";
import { BrowserRouter as Router, Routes, Route, Link } from "react-router-dom"; // Import Link instead of <a>
import { CartProvider, useCart } from "./Context/CartContext";
import HomePage from "./Components/Home";
import CartPage from "./Components/CartPage";
import CheckoutPage from "./Components/Checkout";
...
const Navbar = () => {
    ...
    return (
        <nav className="bg-gray-800 text-white p-4 flex justify-between items-center">
            <Link to="/" className="hover:underline font-semibold">Home</Link> {/* Use Link here */}
            <Link to="/cart" className="hover:underline font-semibold relative">
                Cart
                {cart.length > 0 && (
                    <span className="absolute top-0 right-0 transform translate-x-2 -translate-y-2 bg-red-500 text-white text-sm font-bold px-2 py-0.5 rounded-full">
                        {cart.length}
            ...
            <Link to="/checkout" className="hover:underline font-semibold">Checkout</Link> {/* Use Link here */}
        </nav>
    );
};
